			...
			<tr>
				...
				<td>
					...
					<table width="100%">
						...
						<tr>
							<td><i class="far fa-comment-dots"></i>&nbsp;<b class="userid">{{nickname}}</b></td>
							<td align="right">
...
<script type="text/javascript">
	$(function(){
		...
		//수정버튼 눌렀을 때
		$('#btnBrdModify').on('click',function(e){
			e.preventDefault();
			const bnum=${bdto.bnum};
			//userid체크
...
<div class="container">
	<table width="100%">
		...
		<tr>
			<td colspan="2"><i class="fas fa-user"></i>&nbsp;${bdto.nickname}&nbsp;&nbsp;<i class="fas fa-eye"></i> ${bdto.readcount}</td>
		</tr>
		<tr>
		...
		<c:if test="${!empty bflist}">
			<tr>
				<td colspan="2" class="file-padding">
					<c:forEach var="file" items="${bflist}">
						<a href="${path}/uploadimg/${file.filename}">${file.filename}</a><br>
					</c:forEach>
				</td>
			</tr>
		</c:if>
		...
		<tr>
			<td colspan="2" align="right">
				<c:if test="${bdto.userid==sessionScope.userid}">
					<button id="btnBrdModify" class="btn-custom">수정</button>
